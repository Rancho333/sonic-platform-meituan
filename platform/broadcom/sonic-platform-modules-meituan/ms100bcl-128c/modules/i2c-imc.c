...
static u32 imc_func(struct i2c_adapter *adapter)
{
	...
	if (ch->can_write)
		return I2C_FUNC_SMBUS_BYTE_DATA | I2C_FUNC_SMBUS_WORD_DATA | I2C_FUNC_SMBUS_QUICK;
	else
...
static int imc_init_channel(struct imc_priv *priv, int i, int socket)
{
  ...
  pci_read_config_dword(priv->pci_dev, SMBCNTL(i), &val);
	val &= ~(SMBCNTL_DIS_WRT);
	printk(KERN_INFO "write 0x%x to reg[0x%x]\n", val, SMBCNTL(i));
	pci_write_config_dword(priv->pci_dev, SMBCNTL(i), &val);
	pci_read_config_dword(priv->pci_dev, SMBCNTL(i), &val);
  printk(KERN_INFO "reg[0x%x]=0x%x \n", SMBCNTL(i), &val);
	ch->can_write = !(val & SMBCNTL_DIS_WRT);
  printk(KERN_INFO "can_write=%d \n", ch->can_write);
